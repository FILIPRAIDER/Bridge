...
export async function sendInvitationEmail(params: SendInvitationEmailParams) {
  try {
    console.log('🚀 [sendInvitationEmail] INICIANDO con params:', {
      email: params.email,
      hasToken: !!params.token,
      teamId: params.teamId,
      invitedByUserId: params.invitedByUserId
    })
    console.log('🔑 [sendInvitationEmail] RESEND_API_KEY configured:', !!process.env.RESEND_API_KEY)
    ...
    // 🔧 En desarrollo, forzar envío a email específico (útil para testing)
    const finalRecipient = RESEND_DEV_FORCE_TO || email
    const isDevMode = !!RESEND_DEV_FORCE_TO

    console.log('📧 [sendInvitationEmail] Llamando a Resend con:', {
      from: RESEND_FROM,
      to: finalRecipient,
      originalTo: email,
      isDevMode,
...
function generateInvitationEmailHTML({
  ...
  // Banner de desarrollo (si aplica)
  const devBanner = isDevMode ? `
    <div style="background-color: #fbbf24; padding: 12px 20px; text-align: center; border-bottom: 2px solid #f59e0b; margin-bottom: -1px;">
      <p style="margin: 0; font-size: 13px; font-weight: 600; color: #78350f;">
        🔧 MODO DESARROLLO - Email original: ${inviteeEmail} → Enviado a: ${devRecipient}
      </p>
    </div>
  ` : '';
